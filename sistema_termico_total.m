function dYdt = sistema_termico_total(t, Y, p, Tinit_K)
    % --- sistema_termico_total_corrigido.m (v6.3 - Modelo de Radiação Interna Completo) ---
    ...
    if Pel > 0 && m_res_atual > 1
        ...
        Q_gen_motor = 0; Q_gen_gerador = 0; m_dot_combustivel = 0;
    end
    Q_gen_bateria = 0;
    ...
    % 1. Calcular a temperatura média das superfícies que rodeiam o motor
    A_total_int = p.A_parede_leste_motor + p.A_parede_oeste_motor + p.A_parede_sul_motor + p.A_parede_teto_motor + p.A_parede_motor;
    T_sup_media_K4 = (p.A_parede_leste_motor * Tparede_leste_motor^4 + ...
                      p.A_parede_oeste_motor * Tparede_oeste_motor^4 + ...
                      p.A_parede_sul_motor * Tparede_sul_motor^4 + ...
                      p.A_parede_teto_motor * Tparede_teto_motor^4 + ...
                      p.A_parede_motor * Tparede_motor_int^4) / A_total_int;
    % 2. Calcular a troca de calor líquida total por radiação do motor
    Q_rad_total_motor = p.sigma * p.epsilon * p.A_motor * (Tmotor^4 - T_sup_media_K4);

    % 3. Distribuir essa energia para cada superfície, com base na sua área
    Q_rad_para_leste = Q_rad_total_motor * (p.A_parede_leste_motor / A_total_int);
    Q_rad_para_oeste = Q_rad_total_motor * (p.A_parede_oeste_motor / A_total_int);
    Q_rad_para_sul = Q_rad_total_motor * (p.A_parede_sul_motor / A_total_int);
    Q_rad_para_teto = Q_rad_total_motor * (p.A_parede_teto_motor / A_total_int);
    Q_rad_para_divisoria = Q_rad_total_motor * (p.A_parede_motor / A_total_int);

    % --- Outras trocas de calor ---
    ...
    R_conv_chao = 1 / (h_int * p.A_base_motor);
    R_cond_chao = 1 / (p.S_motor * p.k_solo);
    Q_chao_motor = (Tar_motor - p.Tinf) / (R_conv_chao + R_cond_chao);
    ...
    dT_parede_teto_motor = (Q_rad_para_teto + Q_conv_in_teto - Q_conv_out_teto - Q_rad_out_teto + Q_solar_teto) / (p.m_parede_teto_motor * p.cp_parede_teto_motor);
    dT_parede_leste_motor = (Q_rad_para_leste + Q_conv_in_leste - Q_conv_out_leste - Q_rad_out_leste + Q_solar_leste) / (p.m_parede_leste_motor * p.cp_parede_leste_motor);
    dT_parede_oeste_motor = (Q_rad_para_oeste + Q_conv_in_oeste - Q_conv_out_oeste - Q_rad_out_oeste + Q_solar_oeste) / (p.m_parede_oeste_motor * p.cp_parede_oeste_motor);
    dT_parede_sul_motor = (Q_rad_para_sul + Q_conv_in_sul - Q_conv_out_sul - Q_rad_out_sul + Q_solar_sul) / (p.m_parede_sul_motor * p.cp_parede_sul_motor);
