function dYdt = sistema_termico_total(t, Y, p, Tinit_K)
    ...
    Tchao_motor = Y(20); Tchao_reserv = Y(21); % NOVOS ESTADOS
    ...
    A_cross_section = 2.5*2.5; v_int = 3;
    ...
    h_ext_teto_mot=calc_h_convec(L_teto_mot,0,Tparede_teto_motor,Tamb,'plate','horizontal_up'); h_ext_leste_mot=calc_h_convec(L_parede_vert,0,Tparede_leste_motor,Tamb,'plate','vertical');
    h_ext_oeste_mot=calc_h_convec(L_parede_vert,0,Tparede_oeste_motor,Tamb,'plate','vertical'); h_ext_sul_mot=calc_h_convec(L_parede_vert,0,Tparede_sul_motor,Tamb,'plate','vertical');
    h_ext_norte_res=calc_h_convec(L_parede_vert,0,Tparede_norte_res,Tamb,'plate','vertical'); h_ext_leste_res=calc_h_convec(L_parede_vert,0,Tparede_leste_res,Tamb,'plate','vertical');
    h_ext_oeste_res=calc_h_convec(L_parede_vert,0,Tparede_oeste_res,Tamb,'plate','vertical'); h_ext_teto_res=calc_h_convec(L_teto_res,0,Tparede_teto_res,Tamb,'plate','horizontal_up');
    ...
    A_total_int=p.A_parede_leste_motor+p.A_parede_oeste_motor+p.A_parede_sul_motor+p.A_parede_teto_motor+p.A_parede_motor+p.A_base_motor;
    T_sup_media_K4=(p.A_parede_leste_motor*Tparede_leste_motor^4 + p.A_parede_oeste_motor*Tparede_oeste_motor^4 + p.A_parede_sul_motor*Tparede_sul_motor^4 + p.A_parede_teto_motor*Tparede_teto_motor^4 + p.A_parede_motor*Tparede_motor_int^4 + p.A_base_motor*Tchao_motor^4) / A_total_int;
    ...
    Q_rad_para_chao = Q_rad_total_motor * (p.A_base_motor / A_total_int);
    ...
    Q_conv_ar_chao_motor = h_int_placa_vert*p.A_base_motor*(Tar_motor-Tchao_motor);
    Q_cond_chao_solo_motor = p.S_motor*p.k_solo*(Tchao_motor-p.Tinf);
    Q_conv_ar_chao_reserv = h_int_placa_vert*p.A_base_res*(Tar_res-Tchao_reserv);
    Q_cond_chao_solo_reserv = p.S_res*p.k_solo*(Tchao_reserv-p.Tinf);
    ...
    dT_ar_res = (Q_conv_parede_div_res_ar + Q_conv_res_ar + Q_vent_res - Q_conv_ar_chao_reserv - Q_conv_in_norte_res - Q_conv_in_leste_res - Q_conv_in_oeste_res - Q_conv_in_teto_res) / (p.m_ar_res * p.cp_ar_res);
    dTmotor = (Q_gen_motor - Q_radiador - Q_conv_motor_ar - Q_rad_total_motor) / (p.m_motor * p.cp_motor);
    dT_ar_motor = (Q_conv_motor_ar + Q_conv_gerador_ar + Q_conv_bateria_ar + Q_vent_motor - Q_conv_ar_parede_div_motor - Q_conv_in_teto - Q_conv_in_leste - Q_conv_in_oeste - Q_conv_in_sul - Q_conv_ar_chao_motor) / (p.m_ar * p.cp_ar);
    ...
    dTchao_motor_dt = (Q_conv_ar_chao_motor + Q_rad_para_chao - Q_cond_chao_solo_motor) / (p.m_chao_motor * p.cp_chao_motor);
    dTchao_reserv_dt = (Q_conv_ar_chao_reserv - Q_cond_chao_solo_reserv) / (p.m_chao_reserv * p.cp_chao_reserv);

    dTgerador = (-Q_conv_gerador_ar) / (p.m_gerador * p.cp_gerador);
